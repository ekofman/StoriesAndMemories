...
audio {{
  width: 100%;
  margin-bottom: 10px;
  max-height: 20px;         /* set a maximum height to make it thinner */
...
"""
    ...
    <script>
        ...
        // Mapping from index to image info
        ...
        // Function to jump to a section and update TOC and image.
        function jumpToSection(sectionId, seconds) {
            ...
            updateToc();
            // Update image if there's an associated image for this section.
        ...
        // Attach a scroll event listener to the transcript (right column) to update TOC.
...
if (scrollDiv) {
    scrollDiv.addEventListener("scroll", function() {
        ...
        // Calculate the closest section among all mainPoints
        ...
        // Update the TOC if a new section is closest.
        if (closestSectionId && closestSectionId !== currentSectionId) {
            currentSectionId = closestSectionId;
            updateToc();
        }
        // If there are image sections, update the image if the current section has one.
        if (imageSections.length > 0) {
            for (var i = 0; i < imageSections.length; i++) {
                if (imageSections[i].section_id === currentSectionId) {
                    currentImageIndex = i;
                    updateImage();
                    break;
                }
