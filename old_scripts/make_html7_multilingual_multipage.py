...
image_dict = {
	"young_ester": {
		"image_path": "images/ester_as_young_woman.png",
		"image_subtitle": "Ester as a young woman"
	},
	"boarding_lisbon": {
		"image_path": "images/boarding_in_lisbon.png",
		"image_subtitle": "Waiting to board in Lisbon"
	} 
}
...
# (You must supply the correct main points for each interview.)
main_points_dict = {
    "MaritaInterview5_4_20": [
        {
            "id": "section-1",
...
def generate_page_with_nav(config, main_points_list, all_configs):
    # Decide which transcript file to use for the TOC anchors.
    # If a Spanish transcript is available, use that; otherwise, use the English transcript.
    if "spanish_transcript_file" in config:
        toc_file = config["spanish_transcript_file"]
    else:
        toc_file = config.get("transcript_file", "")
    
    # (We process toc_file for TOC anchors if needed; here we’re just ensuring main_points_list is used for the TOC links.)
    ...
    if "transcript_file" in config:
        ...
        # If no English transcript is provided, fall back to the Spanish transcript.
        processed_en = process_transcript(config["spanish_transcript_file"], main_points_list)
    else:
        processed_en = ""
    
    # Process the Spanish transcript if available.
    processed_es = ""
    if "spanish_transcript_file" in config:
...
<body>
    ...
    <div id="container">
        ...
        <div id="toc">
            ...
            <ul>
"""
    # Build the TOC using the main_points_list.
...
"""
    ...
    <script>
        ...
        // then adjust the Spanish transcript scroll to match as closely as possible.
        function jumpToSection(sectionId, seconds) {
            ...
            // For Spanish, use the anchor inside its container.
